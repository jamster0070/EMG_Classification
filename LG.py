...
def extract_features(segment):
    ...
    fft_raw1 = fft(segment['raw1'].values)
    fft_raw2 = fft(segment['raw2'].values)
    fft_raw3 = fft(segment['raw3'].values)
...
# Hyperparameters
# Using 'multinomial' for multi-class classification (Softmax Regression).
...
report_log_reg = classification_report(y_test, y_pred_log_reg, target_names=le.classes_)

print(f"Softmax Regression Accuracy: {accuracy_log_reg}")
...
labels = np.asarray([f'{value}' for value in conf_matrix_log_reg.flatten()]).reshape(conf_matrix_log_reg.shape)

plt.figure(figsize=(10, 7))
sns.heatmap(conf_matrix_log_reg, annot=labels, fmt="", cmap="Blues", xticklabels=le.classes_, yticklabels=le.classes_)
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Confusion Matrix_Softmax')
plt.show()